resource "vsphere_virtual_machine" "ZabbixDBSlave" {
  name             = "${var.source_pfx}-ZabbixDBSlave-${count.index}"
  num_cpus         = var.zabbixdbs_vm_params["vcpu"]
  count = 2
  memory           = var.zabbixdbs_vm_params["ram"]
  ...
  disk {
    label = "disk0"
    size = var.zabbixdbs_vm_params["disk_size"]
    thin_provisioned = data.vsphere_virtual_machine.template.disks.0.thin_provisioned
  }

  disk {
    label = "disk1"
    size = var.zabbixdbs_vm_params["db"]
    unit_number = 1
  ...
  provisioner "remote-exec" {
     ...
     connection {
       ...
       user     = "root"
       password = "P@ssw0rd"
       host     = "${var.network_params["based_ip"]}${var.zabbixdbs_ipaddress[count.index]}"
     }
   }
}
